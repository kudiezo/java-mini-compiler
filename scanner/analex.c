...
void main(){
    //Inicializando lista onde v�o estar as palavras reservadas
    keywords = iniListaKeywords();
    //Importando as palavras reservadas para a lista de keywords
    keywords = importarKeyWords(keywords);

    //Inicializando lista de erros l�xicos
    listaLexerros = iniListaLexerros();

    //Abertura do arquivo de c�digo fonte (entrada)
    arquivoCodFonte = fopen("codigoFonte.txt", "r");

    if(arquivoCodFonte == NULL)
        printf("Nao foi possivel abrir o arquivo de c�digo fonte.");
    else {
        printf("\n|\ %30s | %30s | %30s |\n", "LEXEMA", "TOKEN", "LINHA");
        printf("+--------------------------------+--------------------------------+--------------------------------+\n");
        do {
            lexico = analex(arquivoCodFonte);
            printf("| %30s | %30s | %30d |\n", lexico.lexema, lexico.token, lexico.linha);
            printf("+--------------------------------+--------------------------------+--------------------------------+\n");
            //if(c == '\n') linha++;
            zerarVetor();
            lexico.lexema = NULL;
        } while(lexico.token != EOF);
    }
